...
### 12.3.1 Rebuild cmake and ninja against libc++

`cmake` and `ninja` are both written in C++, and both are transitive dependencies of `llvm` itself (`llvm` depends on `cmake`, `ninja`, and `libffi` to build). That means they were compiled back in 12.1, while `make.conf` still pointed at the Pass 2 GCC (11.2) — the only C++ compiler that existed at that point. Their recipes don't pin a compiler explicitly, so they picked up whatever was on `$PATH` at build time, and linked against GCC's `libstdc++`/`libgcc_s` as a result. That's invisible for now: GCC is still on disk, so `libstdc++.so.6`/`libgcc_s.so.1` still resolve at runtime. It stops being invisible the moment 12.5 deletes GCC — `cmake`/`ninja` would fail immediately with `Error loading shared library libstdc++.so.6: No such file or directory`, since nothing since 12.1 ever gave them a reason to be rebuilt.
